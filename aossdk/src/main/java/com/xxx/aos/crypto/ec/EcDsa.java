...
import com.xxx.aos.crypto.Hmac;
...
public class EcDsa {
    ...
    private static BigInteger deterministicGenerateK(CurveParam curveParam, byte[] hash, BigInteger d, SigChecker checker, int nonce ){
        if ( nonce > 0 ){
            ...
            //新版，	Really Deterministic K Generating
        ...
        // Step d
        AosByteWriter bwD = new AosByteWriter(32 + 1 + 32 + 32);
        bwD.putBytes(v);
        bwD.put((byte) 0x00 );
        bwD.putBytes( dBytes );
        bwD.putBytes(hash);
        ...
        // Step f
        AosByteWriter bwF = new AosByteWriter(32 + 1 + 32 + 32);
        bwF.putBytes(v);
        bwF.put((byte) 0x01 );
        bwF.putBytes(dBytes);
        bwF.putBytes(hash);
        ...
        while ((t.signum() <= 0) || (t.compareTo( curveParam.n()) >= 0) || !checker.checkSignature(curveParam, t)) {
            AosByteWriter bwH = new AosByteWriter(32 + 1);
            bwH.putBytes(v);
            bwH.put((byte) 0x00);
    ...
    public static EcSignature sign(Sha256 hash, AosPrivateKey key ) {
        ...
        AosPublicKey pubKey = key.getPublicKey();

        for (int i = 0; i < 4; i++) {
            AosPublicKey recovered = recoverPubKey(curveParam, data, signature, i);
    ...
    public static AosPublicKey recoverPubKey(byte[] messageSigned, EcSignature signature ) {
        return recoverPubKey( signature.curveParam, messageSigned, signature, signature.recId);
    }

    public static AosPublicKey recoverPubKey(CurveParam curveParam, byte[] messageSigned, EcSignature signature, int recId ) {
        ...
        return new AosPublicKey( q.getEncoded() );
    }
